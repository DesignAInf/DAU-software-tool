...
## ==============================================================
## Per-agent precision values for the Rigid Designer scenario
## ==============================================================
_BETA_DSG  = 0.05   ## near-zero → Designer ignores observations
_GAMMA_DSG = 10.0   ## high → Designer always picks the same policy
_BETA_ART  = 1.0    ## baseline
_GAMMA_ART = 1.0
_BETA_USR  = 1.0
_GAMMA_USR = 1.0
...
## SCENARIO – Rigid Designer: strong prior on (FOCUSED, SHORT), β≈0, γ>>1
_D_DSG_rigid = utils.obj_array(2)
_D_DSG_rigid[0] = np.array([0.95, 0.05])                              ## FOCUSED dominant
_n_s2 = len(_labDsg['s̆']['s̆ᴬʳᵗᴰˢᵍ₂'])
_D_DSG_rigid[1] = np.array([0.90] + [0.10/(_n_s2-1)]*(_n_s2-1))     ## SHORT dominant
_agtDsg = PrecisionAgent(_agtDsg, beta=_BETA_DSG, gamma=_GAMMA_DSG, D_override=_D_DSG_rigid)
...
_agtArt = PrecisionAgent(_agtArt, beta=_BETA_ART, gamma=_GAMMA_ART)
...
_agtUsr = PrecisionAgent(_agtUsr, beta=_BETA_USR, gamma=_GAMMA_USR)
...
def simulate(T):
    ...
    visualize_EFE_timeseries(_efe_history_Dsg, _efe_history_Art, _efe_history_Usr)
...
def visualize_EFE_timeseries(efe_dsg, efe_art, efe_usr):
    ...
    ax.plot(t, dsg, color="#E05C5C", linewidth=2,   label=f"Designer   (β={_BETA_DSG}, γ={_GAMMA_DSG})")
    ax.plot(t, art, color="#4C9BE8", linewidth=2,   label=f"Smartphone (β={_BETA_ART}, γ={_GAMMA_ART})")
    ax.plot(t, usr, color="#52B788", linewidth=2,   label=f"User       (β={_BETA_USR}, γ={_GAMMA_USR})")

    ax.set_xlabel("Timestep  $t$", fontsize=12)
    ax.set_ylabel("EFE$_{selected}(t)$", fontsize=12)
    ax.set_title("Expected Free Energy over time — Rigid Designer scenario",
